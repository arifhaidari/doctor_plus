...
class _AddFamilyMemberState extends State<AddFamilyMember> {
  ...
  void _createDistrictOptionalList(CityModel cityObject) async {
    final _districtResponse =
        await HttpService().getRequest(endPoint: PATIENT_DISTRICT_LIST + "?q=${cityObject.id}");
  ...
  void initState() {
    super.initState();
    _theImage = const AssetImage(GlobalVariable.MALE_ICON);
    _getInitialData();
  }
  ...
  void _getInitialData() async {
    ...
    final _relativeRelationResponse =
        await HttpService().getRequest(endPoint: PATIENT_RELATIVE_LIST, isAuth: true);

    if (!_relativeRelationResponse.error) {
      try {
        setState(() {
          _relativeRelationResponse.data.forEach((element) {
      ...
      child: Scaffold(
          ...
          appBar: AppBar(
            ...
            leading: IconButton(
              ...
              onPressed: () {
                Navigator.of(context).pop({'is_added': false});
              },
            ),
          ),
          body: Builder(
  ...
  Future<void> _addFamilyMember(BuildContext ctx) async {
    ...
    try {
      ...
      if (!profileSettingResponse.error) {
        if (profileSettingResponse.data['message'] == 'success') {
          // http://10.0.2.2:8000/media/user_avatar/relative/qzi-1_scaled_image_picker412046288.jpg
          ...
          Navigator.of(context).pop({
            'is_added': true,
            'id': profileSettingResponse.data['id'],
      ...
      print('valu of e in erororororooror');
      print(e);
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            // image: DecorationImage(image: _theImage),
            image: _image != null
